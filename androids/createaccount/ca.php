...
if (isset($_COOKIE['account'])) {
    $dname = $_COOKIE['account'];
    $headDisplay='Developer Account';
    $but='Sign-In';
    if (isset($_COOKIE['pwd'])) {
        if (file_exists('sub/' . $dname . ".l")) {
            $myfile = fopen('sub/' . $dname . ".l", "r");
            $pwd = fgets($myfile);
            fclose($myfile);
            $pwd2 = $_COOKIE['pwd'];
            $pwd = str_replace("\n","",$pwd);
            if (strcasecmp($pwd2, $pwd) == 0) {
                header('Location: https://toremetal.000webhostapp.com/androids/submit.php');
            } else {
                $ace = 'Invalid password! Please try again.';
            }//header('Location: https://toremetal.000webhostapp.com/androids/submit.php');
        } else {
            $myfile = fopen('sub/' . $dname . ".l", "w");
            $pwd = $_COOKIE['pwd'];
            $ace = 'existing account';
            fwrite($myfile, $pwd . "\n");
            fwrite($myfile, 'existing account');
            fclose($myfile);
            header('Location: https://toremetal.com/androids/submit.php');
        }
    } else {
        if (file_exists('sub/' . $dname . ".l")) {
            $myfile = fopen('sub/' . $dname . ".l", "r");
    ...
    $pwd = '';
    $ace = 'New Account';
    $headDisplay='Create a Developer Account';
    $but='Create Account/Sign-In';
}

$hst = $_SERVER['HTTP_HOST'];
$req = $_SERVER['REQUEST_URI'];
//$reqe = str_replace("/androids/?=","",$req);
if (substr($req, 0, 32) == '/androids/submit.php?cookie=app&') {
$xcz = str_replace("/androids/submit.php?cookie=app","",$req);
    $x = 0;
    $z = 0;
    $q = 0;
    $r = 0;
//$myfile = fopen("vc.tmp", "w") or die("ERROR!");

while($x <= strlen($xcz)) {
  $fd=substr($req, (int)$x, 1);
  if (strcasecmp($fd, '=') == 0) {
      $z = $x;
      $z++;
    $q = 1;
  } else if (strcasecmp($fd, '&') == 0) {
    $q = 0;
    $r--;
  echo substr($req, (int)$z, (int)$r) . "<br>";
    $r = 0;
      //fwrite($myfile, substr($req, $z, $r) . "\n");
  }
  if (strcasecmp($q, '1') == 0) {$r++;}
  $x++;
...
//} else if (isset($_COOKIE[$cookie_name])) {
 // list($developer,$app,$package,$youtube,$androidVersion,$shortDescription,$description,$image) = explode("|", json_decode($_COOKIE[$cookie_name], true));
}
//$myfile = fopen($reqe . ".txt", "r") or die("ERROR!");
...
<span class="h1 block bold caps my1"><?=$headDisplay?></span><hr/>
...
<div class="col-lg-8 col-md-8 col-sm-12 width">
                        <div class="address">
                            ...
                            <h4>*Apps must be available on Google Play*</h4>
<?=$ace?>
                            <form id="myForm"><!--style='background-color:red'--> 
                                <div class="row"> 
 <!--<input style="width:0%" class="hidden" placeholder="app" type="text" value="app" name="cookie">-->
                                    <div class="col-sm-12">
<input style="width:100%" id='dev' class="contactus" placeholder="Developer Name" type="text" name="developer" value="<?=$dname?>">
                                    </div>
                                    <div class="col-sm-12">
<input style="width:100%" id='pwd' class="contactus" placeholder="password" type="password" name="pwd">
                                    </div>
                                    <!--<div class="col-sm-12">
<input class="contactus" id='pac' style="width:100%" placeholder="APK-Name(com.developer.app)" type="text" name="package">
                                    </div>
                                    <div class="col-sm-12">
<input class="contactus" id='tub' style="width:100%" placeholder="(YT)Video Embed Link" type="url" name="utube">
                                    </div>-->
                                    <!--<div class="col-sm-12">
<input class="contactus" id='api' style="width:100%" placeholder="API-Level(4.4,5.0,11.0)" type="number" name="api">
                                    </div>-->
                                    <!--<div class="col-sm-12">
<input class="contactus" id='img' style="width:100%" placeholder="Image Embed Link" type="url" name="image">
                                    </div>-->

                                    <!--<div class="col-sm-12">-->
                                    <!--</div>-->
                                    <!--<div class="col-sm-12">
<textarea class="textarea" id='fdes' style="width:100%" placeholder="Full Description" type="text" name="Description"></textarea>
                                    </div>-->
                                    <div class="col-sm-12">
<!--<button onclick="setCookie('app','value',0)" formaction="https://toremetal.000webhostapp.com/androids/ca.php" method='GET' class="send">Submit</button>-->
<!--<button formaction="mailto:toremetal@outlook.com" class="send">Send</button>-->
    <!--onclick='document.getElementById("myForm").reset()'-->
<button class="send" onclick="setACookie('account', 'b', 2147483647)" ><?=$but?></button>
...
<!--<button style="width:100%"  onclick="setCookie('app','value',0)" action="/androids/">test</button>-->
                        </div>
                    </div>
<script>
function dName() {
    var pw = '<?=str_replace("\n","",$pwd)?>';
    var h = '<?=$hst?>';
    var xd = document.getElementById("dev").value;
    if (pw == '') {
        ...
        document.cookie = 'pwd' + "=" + (pwd || "")  + expires + "; path=/androids/;domain=<?=$hst?>;SameSite=Strict";
    } else {
        var pwd = document.getElementById("pwd").value;
        var expires = "";
        var date = new Date();
        date.setTime(date.getTime() + 1000);
        expires = "; expires=" + date.toUTCString();
    document.cookie = 'pwd1' + "=" + (pwd || "")  + expires + "; path=/androids/;domain=<?=$hst?>;SameSite=Strict";
    }
    return xd;
}
function setACookie(name,value,days) {
    xa=document.getElementById("pwd").value;
    dd=dName();
    if (dd == "")  {
        document.getElementById("res").value='Please enter a value';
    } else {
        if ('<?=str_replace("\n","",$pwd)?>' == '') {
        var expires = "";
        if (days) {
        ...
        document.cookie = name + "=" + (dd || "")  + expires + "; path=/;domain=<?=$hst?>;SameSite=Strict";
        document.getElementById("myForm").reset();
        document.getElementById("res").value='Saved Successfully';
        } else if ('<?=str_replace("\n","",$pwd)?>' == xa) {
        var expires = "";
        if (days) {
            var date = new Date();
        ...
        document.getElementById("res").value='Save failed:Account exists, incorrect password, or other error';
    }
...
<footer class="ampstart-footer flex flex-column items-center px3 "><!-- style="background-color:#42577e80;"-->
   <!-- Copyright -->
   <div class="container1" align="center" style="background-color:#49494980"><br>

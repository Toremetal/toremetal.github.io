...
$reqes = str_replace("/androids/?=","",$req);
$reqe = str_replace("%20","-",$reqes);
$reqe = str_replace(" ","-",$reqes);
if (strcasecmp($req, '/androids/') == 0) {
    header('Location: /home');
} else if (strcasecmp($req, '/androids/?=') == 0) {
    header('Location: /home');
} else if (substr($req, 0, 22) == '/androids/?cookie=app&') {
    if (isset($_COOKIE['app'])) {
        list($developer,$app,$package,$youtube,$androidVersion,$shortDescription,$description,$image) = explode("|", $_COOKIE['app']);//json_decode($_COOKIE['app'], true));
        $fili=str_replace(" ","-",$developer . "-" . $app);
        $myfile = fopen('sub/' . $fili . ".txt", "w");
        $txt = $developer . "\n";
        fwrite($myfile, $txt);
        $txt = $app . "\n";
        fwrite($myfile, $txt);
        $txt = 'com.'.strtolower($developer).'.'.$package . "\n";
        fwrite($myfile, $txt);
        $txt = $youtube . "\n";
        fwrite($myfile, $txt);
        $txt = $androidVersion . "\n";
        ...
        fwrite($myfile, $txt);
        fclose($myfile);
        if (!file_exists('sub/devs/' . $developer)) {
            if (!file_exists('sub/devs.t')) {
                $myfile = fopen('sub/devs.t', 'w');
            } else {
                $myfile = fopen('sub/devs.t', 'a');
            }
            $txt = '\n<h2>'.$developer.'</h2>';
            fwrite($myfile, $txt);
            fclose($myfile);
            $myfile2 = fopen('sub/devs/' . $developer, "w");
        } else {
            $myfile2 = fopen('sub/devs/' . $developer, "a");
        }
        $txt2 = "\n<div class='col-sm-2 col-md-2' style='align:center;background-color:#80808000;'><a href='/androids/?=" . $fili . "'><img title='" . $app . "' src='" . $image . "'/>" . $app . "</a></div>";
        fwrite($myfile2, $txt2);
        fclose($myfile2);
    }
} else if (!file_exists('sub/' . $reqe . ".txt")) {
    header('Location: /');
} else {
    $myfile = fopen('sub/' . $reqe . ".txt", "r");
    $developer=str_replace("\n","",fgets($myfile));
    $app=str_replace("\n","",fgets($myfile));
    $package=str_replace("\n","",fgets($myfile));
    $youtube=str_replace("\n","",fgets($myfile));
    $androidVersion=str_replace("\n","",fgets($myfile));
...
$txt = str_replace("\n", "", str_replace("\r", "", '<p align="center"><b>' . str_replace("\n", "", $developer) . '</b><br><i style="color:red">' . str_replace("\n", "", $app).'</i><br>for <i style="color:#000000">Android™</i> '.str_replace("\n", "",$androidVersion).'+</p><hr /><h4 align="left">Description:</h4><h4 align="center"><i style="color:#00000070">'.str_replace("\n","",$shortDescription).'</i></h4>'));
fwrite($myfile, $txt);
fclose($myfile);

$myfile = fopen("tmp.txt", "r");
$shde=str_replace("\n", "", fgets($myfile));
$shdes=str_replace("\r", "", $shde);
fclose($myfile);

$myfile = fopen("tmp.txt", "w");
$txt = str_replace("\n", "", str_replace("\r", "", '<p align="center"><b>' . str_replace("\n", "", $developer) . '</b><br><i style="color:red">' . str_replace("\n", "", $app).'</i><br>for <i style="color:#000000">Android™</i> '.str_replace("\n", "",$androidVersion).'+</p><hr /><h4 align="left">Description:</h4><h4 align="center"><i style="color:#00000070">'.str_replace("\n","",$shortDescription).'</i></h4><p align="justify">'.str_replace("\n","",$description).'</p>'));
fwrite($myfile, $txt);
fclose($myfile);

$myfile = fopen("tmp.txt", "r");
$lde=str_replace("\n", "", fgets($myfile));
$ldes=str_replace("\r", "", $lde);
fclose($myfile);
...
<head> 
<title><?=str_replace('\n', '', $developer)?> - <?=str_replace("\n","",$app)?></title> 
<meta name="description" content="Download <?=str_replace('\n', '', $app)?> for Android - com.toremetal.pos, Created by <?=str_replace('\n', '', $developer)?>"> 
<meta name="keywords" content="<?=$app?>, <?=str_replace('\n', '', $developer)?>"> 
<meta charset="utf-8"> 
<meta http-equiv="X-UA-Compatible" content="IE=edge"> 
<meta name="viewport" content="width=device-width, initial-scale=1"> 
<meta property="og:url" content="https://toremetal.com/android/<?=str_replace("\n","",$app)?>" /> 
<meta property="og:type" content="website" /> 
<meta property="og:title" content="<?=str_replace("\n","",$app)?> " /> 
<meta property="og:image" content="../img/logo.png" /> 
<meta property="og:description" content="Download <?=str_replace("\n","",$app)?> for Android - <?=str_replace("\n","",$package)?>. Created by <?=str_replace('\n', '', $developer)?>" /> 
...
<link rel="canonical" href="https://toremetal.com/android/?=<?=str_replace("\n","",$app)?>"> 
...
<li><a href="/apps">Apps</a></li><li></li>
        ...
        <div class="banner-main">
            ...
            <div class="container">
                <div class="text-bg relative my0">
                    ...
                    <a style="background-color:#80808000;" target="_blank" href='https://play.google.com/store/apps/details?id=<?=$package?>'>
		...
		<div class="main"> 
			<div class="row"> <div class="col-sm-2 col-md-2">
				...
				<div class="col-sm-12 col-md-8">
				    <div class="card fluid idma contactus" Style="background-color:#FFFFFF00;">
        <div class="section app-content contactus" style="background-color:#FFFFFF90">
					<div id="pic" class="card fluid idma">
<embed src="<?=$image?>" style="height: 100%; width: 100%; align:center;">
 				</div> 
          <div id="posaD" class="detail-small">     <p align='center'><b><?=str_replace('\n', '', $developer)?></b><br><i style='color:red'><?=str_replace("\n","",$app)?></i><br>for <i style='color:#000000'>Android™</i> <?=$androidVersion?>+</p><hr /><h4 align='left'>Description:</h4><h4 align='center'><i style='color:#00000070;'><?=str_replace("\n","",$shortDescription)?></i></h4><p align="justify"><?=$description?></p>
          </div> 
<button id="posaB" onclick="expandPosa()" class="read-more" style="background-color:#00000000;color:red;">Show Less</button>
					<div class="card fluid idma"> 
<iframe class="SameSiteFilter img-fluid " referrerpolicy="SameSite:None; Secure" style="height: 100%; width: 100%" src="<?=str_replace('\n','',$youtube)?>" title="<?=str_replace('\n', '', $developer)?> <?=str_replace('\n','',$app)?>" frameborder="0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; cross-origin-isolated" allowfullscreen></iframe>
					</div> 
...
function expandPosa() {
    if (document.getElementById('posaB').innerHTML == 'Show More') {
        ...
        document.getElementById('posaD').innerHTML = '<?= $ldes ?>'
    } else {
        document.getElementById('posaB').innerHTML = 'Show More';
        document.getElementById('posaD').innerHTML = '<?= $shdes ?>'
    }
}
</script>
<a onclick="document.getElementById('app').innerHTML='Thanks for Checking it out!'" target="_blank" href='https://play.google.com/store/apps/details?id=<?=str_replace("\n","",$package)?>'><img class="zoom img-fluid " width="120px" title='Get it on Google Play' alt='Get it on Google Play' src='/img/gp.png'/></a>
                           <div align="center" id="app"></div>
<script>
function myFunction() {
    document.getElementById('app').innerHTML='Thanks for Checking it out!'
};
</script>
</div>
				</div> 
